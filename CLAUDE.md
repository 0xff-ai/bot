...
- **Config externalization.** Areas, the product description fed to the drafter, and the maintainer list are the consuming repo's vocabulary and live in `.github/bot.yml`, never hardcoded in the bot. Config parsing is strict (`src/config.ts`): unknown keys fail loudly so a typo surfaces instead of being ignored. The one bot-owned area is the deliberate exception: `parseConfig` appends `INTERNAL_AREA` (`src/areas.ts`) last so non-user-facing entries sort after the consumer's areas, which is why `internal` is a reserved id and the drafter never skips (it demotes instead of dropping).
...
- **Structured output, always.** The drafter (`src/llm.ts`) uses structured output (`generateText` + `Output.object` + a Zod schema): the model returns JSON and `Output.object` validates it against the schema. Never "fix" a drafting failure by removing `Output.object` and parsing free text yourself: that silently drops the schema guarantee and is the wrong call. Debug the structured-output path instead. What was learned debugging the OpenCode Go gateway (live, 2026-06):
  ...
  - **Temperature is per-model too, in `temperatureFor`.** We want `temperature: 0` for determinism, but Moonshot's code models (`kimi-*-code`) reject it with `invalid temperature: only 1 is allowed for this model`. `temperatureFor` returns 1 for those and 0 elsewhere; extend it the same way as `supportsJsonSchema` when a new model rejects 0.
...
- The reusable workflow on `main` references the action from `0xff-ai/bot@main` so current workflow changes can be tested without moving `v1`. A breaking change to released inputs or behavior still needs a new major tag, not a silent move of `v1`.
